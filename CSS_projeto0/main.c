...
{
  _DINT(); 				//Desabilita as interrupçoes (por enquanto)
  IE2 |= UCB0RXIE;		//Habilita interrupcao RX
  IE2 |= UCB0TXIE;      //Habilita interrupcao TX
  while (UCB0CTL1 & UCTXSTP)
  {
	  NOP(); //aguarde até o os registradores ficarem livres para fazer o SET
  }
...
void EnviarAssincrono(char saida)
{
    ...
    __bis_SR_register(CPUOFF + GIE);        // LOW POWER MODE e interrupcao
}
